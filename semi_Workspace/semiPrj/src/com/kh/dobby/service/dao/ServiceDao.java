...
//            pstmt.setInt(2, end);
    ...
    public List<ServiceVo> listPopUser(Connection conn) {
        //TODO
        String sql = "SELECT * FROM( SELECT ROWNUM NUM, S.SERVICE_NO, S.USER_NO, Z.CNT, S.SERVICE_TITLE, S.SERVICE_INTRO, S.IMAGE_LINK, S.CHARGE, S.CHARGE_UNIT_NO, C.CHARGE_UNIT, U.NICK FROM SERVICE_INFO S INNER JOIN (SELECT SERVICE_NO, COUNT(*) CNT FROM ZZIM GROUP BY SERVICE_NO) Z ON S.SERVICE_NO = Z.SERVICE_NO JOIN \"USER\" U ON S.USER_NO = U.USER_NO JOIN CHARGE_UNIT C ON S.CHARGE_UNIT_NO = C.CHARGE_UNIT_NO WHERE ROWNUM<=10) ORDER BY CNT DESC";
